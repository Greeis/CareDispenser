...
void acertaRelogio()  
{
  ...
  if (i==4)                                   //aceita apenas 4 caracteres
  {
    ...
    String MR="";MR=MR+AA.charAt(2)+AA.charAt(3);   //os dois ultimos digitos
    AA = HR + ":" + MR;                      //AA no formato HR:MR
    byte hr=HR.toInt();
    byte mr=MR.toInt();
    rtc.setTime(hr, mr, 0);     //altera o horario do relogio RTC
  }else{
    bluetooth.println("Tem que ser HHMM,");
    bluetooth.println(" 4 digitos numericos");
  }
}
...
{
  String h1;
  bluetooth.println("Horario do alarme 1 em HHMM");
  lcd.clear();
  lcd.print("Alarme 1 (HHMM) ");
  while(!bluetooth.available() > 0){true;}//-- espera entrada via bluetooth 
  while(bluetooth.available() > 0)
  { h1 = h1+(char)bluetooth.read();}          //soma caracteres em "AA"
  Serial.println("Alarme 1" + h1);
  byte i=h1.length();  //verifica 4 caracteres
  Serial.println(i);
  if (i==4)                                   //aceita apenas 4 caracteres
  {
    //................................................ coloca ":" entre hora e minuto
    String HR="";HR=HR+h1.charAt(0)+h1.charAt(1);   //os primeiros dois digitos
    String MR="";MR=MR+h1.charAt(2)+h1.charAt(3);   //os dois ultimos digitos
    ALARM1 = HR + ":" + MR;                      //AA no formato HR:MR    
  }else{
    bluetooth.println("Tem que ser HHMM,");
    bluetooth.println(" 4 digitos numericos");
  }
...
{
  String r;
  bluetooth.println("Informe o horário do alarme em HHMM");
  lcd.clear();
  lcd.print("Recarga  (HHMM) ");
  while(!bluetooth.available() > 0){true;}          //espera entrada via bluetooth 
  while(bluetooth.available() > 0)
  { r = r+(char)bluetooth.read();}                  //soma caracteres em "r"
  Serial.println("  Recarga" + r);
  byte i=r.length();  //verifica 4 caracteres
  Serial.println(i);
  if (i==4)                                         //aceita apenas 4 caracteres
  {
    //................................................ coloca ":" entre hora e minuto
    String HR="";HR=HR+r.charAt(0)+r.charAt(1);     //os primeiros dois digitos
    String MR="";MR=MR+r.charAt(2)+r.charAt(3);     //os dois ultimos digitos
    RECARGA = HR + ":" + MR;                        //RECARGA no formato HR:MR    
  } else{
    bluetooth.println("Precisar que ser HHMM, ");
    bluetooth.println("4 digitos numericos");
  }
}